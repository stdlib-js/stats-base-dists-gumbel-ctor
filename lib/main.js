...
function Gumbel() {
	...
	if ( arguments.length ) {
		...
		if ( !isNumber( mu ) || isnan( mu ) ) {
			throw new TypeError( format( '11v8p,HM', mu ) );
		}
		if ( !isPositive( beta ) ) {
			throw new TypeError( format( '11v7c,HN', beta ) );
		}
	} else {
		mu = 0.0;
		beta = 1.0;
	...
	defineProperty( this, 'mu', {
		...
		'set': function set( value ) {
			if ( !isNumber( value ) || isnan( value ) ) {
				throw new TypeError( format( '11v8d,H8', value ) );
			}
			mu = value;
		}
	});
	defineProperty( this, 'beta', {
		...
		'set': function set( value ) {
			if ( !isPositive( value ) ) {
				throw new TypeError( format( '11v8k,HE', value ) );
			}
			beta = value;
		}
	});
